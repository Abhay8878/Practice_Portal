...
// import { restorationShade } from "../../form/product/ShadeDetails";
...
export default function OrderDetailsDialog({
  ...
  const has3DImages = (order?.image_3d_urls?.length ?? 0) > 0;
  const { language } = useLanguage();
  const t = strings[language].products.details;
  const productForm = strings[language].products.form;
  const trackingText = strings[language].products.tracking;
            ...
            <div className="space-y-6">
              ...
              {has3DImages && (
                <div className="bg-gradient-to-br from-brand-50 to-white rounded-xl p-6 border border-brand-100 shadow-sm">
                  ...
                  {/* Thumbnails / Switcher */}
                  {(order.image_3d_urls?.length ?? 0) > 1 && (
                    <div className="flex gap-2 mt-2 overflow-x-auto pb-5">
                      {order.image_3d_urls!.map((url, idx) => (
                        <button
                          key={idx}
                        ...
                        >
                          ...
                          <span className="text-[10px] font-medium text-gray-600">
                            {order.image_3d?.[idx]?.file_name?.split(".")[0] ||
                              `Model ${idx + 1}`}
                          </span>
                        </button>
                      ))}
                  ...
                  <div className="w-full h-[400px] border rounded-lg bg-gray-900 overflow-hidden relative">
                    {selectedIndex !== null && order.image_3d_urls ? (
                      <ThreeDimensionalViewer
                        key={`3d-viewer-${selectedIndex}-${order.image_3d_urls[selectedIndex]}`}
                        pathToFile={order.image_3d_urls[selectedIndex]}
                      />
                    ) : (
